...
class PizzaProcessor:
    ...
    def pizza_pick(self, name, crust, num_toppings):
        if not isinstance(name, str):
            return self._error
        if not isinstance(crust, str):
            return self._error
        if not isinstance(num_toppings, int) or not num_toppings <= 10:
            return self._error
        return f"{name} ordered a {crust} crust pizza with {num_toppings} toppings!"
    ...
    def pizza_prepper(self, *toppings):
        ...
        for topping in toppings:
            if isinstance(topping, int):
                if topping > 10:
                    return self._error
            else:
                return self._error
            crust = "stuffed" if order_number % 2 == 1 else "thin"
            pizzas[str(order_number)] = {"crust": crust, "toppings": topping}
            order_number += 1
        print(pizzas)
        return pizzas
    
    def pizza_printer(self, **pizzas):
        result = "Printing all the pizzas!"
        for order_number in pizzas:
            result += f"\nOrder {order_number} is a {pizzas[order_number].get('crust')} crust with {pizzas[order_number].get('toppings')} toppings\n"
        return result
